...
export default function LecturerDashboard() {
  ...
  const onPost = async (e) => {
    ...
    const title = (composerTitle || "").trim();
    if (!title && !html && imagePreviews.length === 0 && docFiles.length === 0 && composerLinks.length === 0) return;

    // Append the links at the bottom in blue + underlined
                            ...
                            {composerLinks.map((u) => (
                              <li key={u} className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    setComposerLinks((links) => links.filter((x) => x !== u))
                                  }
                                >
                                  remove
                                </button>
