                ...
                <div key={r.id} className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    {r.files?.length>0 && (
                      <ul className="mt-2 space-y-1">
                        {r.files.map((f,i)=>(
                          <li key={i} className="flex items-center gap-2">📎 <AttachmentLink att={f} /></li>
                        ))}
                      </ul>
                    )}
                  </div>
...
useEffect(() => {
  ...
  const recompute = () => {
    try {
      setUnreadLecturerResponses(computeUnreadForStudent(me.id));
    } catch {}
  };
  ...
  // ===== Consent wiring (hooks) — no duplicate UI added =====
  const { id: consentStudentId, email: consentEmail } = consentGetIdentity();
  const [consentState, setConsentState] = useState(() =>
    consentLoadFor(consentStudentId, consentEmail)
  );
  useEffect(() => {
    setConsentState(consentLoadFor(consentStudentId, consentEmail));
  }, [consentStudentId, consentEmail]);

  // Bridge: capture checkbox changes from existing UI (e.g., inside StudentAlertsCTA)
  useEffect(() => {
    ...
    const findLabelText = (input) => {
      ...
      // <input id="x" /><label for="x">Text</label>
    ...
    const onChange = (e) => {
      ...
      // Prefer explicit data-consent-key if provided
  ...
  // Video posts saved by admin (localStorage: "videoPosts")
  const [videoPosts, setVideoPosts] = useState(() => {
    const arr = safeParse(localStorage.getItem("videoPosts")) || [];
    return Array.isArray(arr) ? arr : [];
  });

  // keep in sync if another tab/admin updates them
  useEffect(() => {
    const sync = () => {
      const arr = safeParse(localStorage.getItem("videoPosts")) || [];
  ...
  // ⬇️ Replace your current visibleVideos useMemo with this:
const visibleVideos = useMemo(() => {
  ...
  return (videoPosts || [])
    ...
    .filter(p => {
      ...
      // backward compatible: if no videoAudience saved, treat as "all"
      ...
      // "all" (or anything else) => show to all students
      return true;
    })
...
const latestVideo = visibleVideos[0] || null;
  ...
  const facYearKey = `${baseFac}__${user.year}`;
   ...
   // Normalize createdAt (handles number millis or ISO string)
  ...
  const seeded = useMemo(()=>[
    ...
    { id:"p1", createdAt: Date.now()-24*60*60*1000,
      ...
      time:"Yesterday", audience:baseFac, type:"Assignments",
...
// Merge student + lecturer posts for notification/lecturer-toast logic
...
// keep in sync if another tab/lecturer updates them
useEffect(() => {
  const sync = () => {
    const arr = safeParse(localStorage.getItem("lecturerPosts")) || [];
...
const feedCombined = useMemo(() => {
  const a = Array.isArray(posts) ? posts : [];
  const b = Array.isArray(lecturerPosts) ? lecturerPosts : [];
  // Make sure lecturer items have authorType set
  const normalizedLect = b.map(p => (p.authorType ? p : { ...p, authorType: "lecturer" }));
...
useEffect(() => {
  ...
  // Use merged posts (students + lecturers) so the bell counts lecturer posts too
const recomputeUnseen = useMemo(() => () => {
  ...
  window.addEventListener("lecturerPosts:updated", onAnyPosts); // NEW

  return () => {
    window.removeEventListener("storage", onAnyPosts);
    window.removeEventListener("posts:updated", onAnyPosts);
    window.removeEventListener("lecturerPosts:updated", onAnyPosts); // NEW
  };
}, [recomputeUnseen]);
    ...
    {latestVideo ? (
      ...
      <>
        ...
        <p className="text-sm text-slate-600 mt-1 text-center">
          No video yet. When your admin posts a video for Students (or Both), it will
          appear here.
        </p>
      </>
    )}
  ...
  {/* NEW: Contact a Lecturer card under the Purple Alerts CTA */}
<div className="mt-3">
  <div className="w-full border border-slate-200 bg-white rounded-2xl p-4">
    ...
    <div className="mt-2 flex items-center justify-between">
      ...
      {unreadLecturerResponses > 0 ? (
        ...
        <span className="inline-block min-w-[28px] h-7"></span>
      )}
    </div>
  </div>
</div>
